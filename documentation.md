...
Double-click to run, ignore non-signed problems, a dev signature costs money and this is a free open-source project.

Now you have a few ways of appraching this: single PTU analysis, single region analysis, project analysis.

### Single PTU analysis

Open the executable and select 'Single FOV Fit' 

Select 'Fit settings' and populate the fields as follows:

- PTU path: select your .ptu file
    -> To extract the PTU from Leica LAS X, open the lif/lof and navigate to file on the FLIM window and export raw data. 
- IRF method: select 'Machine IRF' (recommended) or 'IRF XLSX' if you have a LAS X export for that PTU. If you have a scatter PTU, select 'Scatter PTU'. If none of these are available, select 'Estimate from decay' and set the FWHM to around 0.3–0.5 ns.
    -> To extract the IRF from Leica LAS X XLSX export, open the lif/lof and navigate to the FLIM window. Right click on the Summed decay/Tail decay and select 'Export to Excel'. This will give you a file with the IRF and decay curves which FLIMKit can use for fitting.

Fitting parameters: 
- Number of exponentials: 1, 2, or 3 (default: 2) - after 3 the math doesn't really work and it's hard to interpret the results biologically, so I don't allow more than 3.
- Lifetime bounds: 0.145–45 ns (default) - these are the bounds for the lifetime components; adjust if you expect very short or long lifetimes
- Fitting mode: Full (default) runs both summed and per-pixel fitting which is needed to generate a FLIM image for th UI. If you just want the global lifetime values for the whole FOV, select 'FAST' to save time. Per-pixel fitting is slow, especially with more exponentials, so only run it if you need the spatial maps.
- Output prefix: this is the prefix for the output files; by default it will save to the same directory as the PTU with the prefix 'flim_out'. You can change this to organize your outputs better.

Mask & thresholding:
- Cell mask uses Otsu thresholding to create a binary mask of the cell regions. This can help exclude background pixels from the fitting. Based on the intensity threshold, should be able to remove sample from background but success is hit-or-miss depending on the intensity of the data. 
- Intensity threshold applies a simple photon count cutoff to exclude low-signal pixels from per-pixel fitting. This can speed up fitting and improve map quality by ignoring noisy pixels, but be careful not to set it too high or you might exclude dim but valid regions.

If you need more flexibility with the fitting parameters (e.g. custom optimisers, cost functions, DE settings), you can set those up in the 'Expert fit settings' tab. These are shared between single FOV and tile stitching pipelines.

### Tile stitching and fitting

Open the executable and select 'Tile Stitch / Fit'

Input files will be the XLIF metadata file and the directory containing the corresponding PTU tiles. The XLIF is generated by Leica LAS X, and it contains the stage coordinates and tile layout, it is found within the Metadata folder in your LAS X project folder. The PTU tiles are the individual FLIM data files for each tile. If there are tiles from multiple different scans in the same directory, the program should be able to sort them out as long as the XLIF and tiles have consistent naming conventions.

Different pipeline modes are available to you:
- Stitch only: this will create a stitched intensity image and a stitched FLIM histogram cube, but it won't run the reconvolution fitting. This is useful if you want to do a quick visual check of the stitching quality or if you want to export the stitched data for use in another software. It will export the flim cube as an npy file. 
- Stitch then fit full ROI pipeline: this will run the full reconvolution fitting pipeline on the stitched mosaic. It will first run a global stitch on the intensity, then place the photons from the tiles onto a stitched canvas, then run a fit on the stitched canvas. Not recommended unless you have a beefy computer and a small number of tiles, as the fitting will be slow and require a lot of available memory (tens of Gbs RAM...). 
- Per-tile fit pipeline: this will stitch the tiles together, but instead of running the fit on the stitched mosaic, it will first dump all photon histograms into a global decay and run an initial fit. Following which, it will run the fit separately on each tile based on the global and then stitch the resulting lifetime maps together. This is much faster and more memory efficient than fitting the whole mosaic at once, and it should give you the same results as fitting the mosaic directly since the fitting is done on the same data, just in smaller chunks. This is the recommended option for most users.

Fitting parameters are the same as for single PTU analysis, however, it is recommended you generate a machine-IRF rather than relying on per-tile IRF xlsx from LAS X, as the machine IRF will be more stable across tiles.

As per single FOV analsysis, a per pixel fit is available. If the option is unticked it will do the fit without fitting by pixel photons. You wont get a FLIM image out of this. However per pixel allow specific exports:
- Intensity image (16-bit tiff): this is the total photon count per pixel, it can be useful for visualising the sample and for masking.
- X weighted images (32-bit tiff): these are the α-weighted or intensity-weighted lifetime images. The lifetime range can be set here too. 
- Individual component τ maps (32-bit tiff): these are the lifetime maps for each component, they can be useful for visualising the spatial distribution of each lifetime component.

Masking and thresholding options are the same as for single FOV analysis, but they will be applied to each tile separately before stitching. Cell detection will not be enabled through the GUI, try through the Python API if you want to use it for tile stitching. It is more efficient that way. 

Tile registration
This is necessary as the stage seems to drift slightly during acquisition, which can cause misalignment between tiles. The stitching algorithm uses phase correlation to estimate the necessary shifts to align the tiles. There are three passes of registration:
1. Column Y drift correction: this corrects for any systematic drift in the Y direction across columns of tiles, which is common in stage scanning.
2. Row Y residual correction: this corrects for any remaining misalignment in the Y direction after the first pass, on a row-by-row basis.
3. Row X backlash correction: this corrects for any misalignment in the X direction due to stage backlash, which can occur when changing direction.
You can set the maximum expected drift in the settings, which will help the algorithm to find the correct alignment. After registration, the tiles are stitched together using a nearest-centre ownership approach, which means that each pixel in the output mosaic is assigned to the tile whose centre is closest to it. This is a simple and efficient method for assembling the mosaic, but it may not be perfect in cases where there is significant overlap between tiles or where the sample has complex structures at the tile borders.

### Batch ROI fitting

Basically the same settings as Tile stitching and fitting to the exception there will be no ROI analysis possible. 
XLIFs should be within a folder and the program will process them sequentially. The output will be a CSV file with the fit summaries for each ROI and a folder per ROI with the intensity image, npy decay and additional lifetime exports (should you so choose).

### ROI analysis (for single FOVs and single stitched ROIs only)

This tab allows you to place ROIs onto the displayed FLIM image and get the mean lifetime values for those ROIs. It is designed for quick exploratory analysis of specific regions of interest within a single FOV or a stitched mosaic.
You can place multiple ROIs on each FOV, and the statistics for all ROIs will be printed to the ROI table. The ROIs are saved as part of the session, so if you save your session and come back to it later, your ROIs will still be there.
Additionally, you can export the ROIs as GeoJSON files, which can be imported into QuPath for further analysis. The GeoJSON files will contain the geometry of the ROIs and the associated statistics, so you can easily integrate them into your QuPath projects.
Likewise, you can import QuPath GeoJSON files to restore ROIs that you have previously defined in QuPath. This allows for a seamless workflow between FLIMKit and QuPath, where you can define ROIs in one software and analyse them in the other. (this is not limited to QuPath, any software that can read GeoJSON files should be able to use the exported ROIs).

Finally you can export the ROIs as a CSV file, which will contain the statistics for each ROI in a tabular format. This can be useful for further analysis in spreadsheet software or for sharing with collaborators who may not use QuPath.

### Phasor analysis

The phasor analysis tab provides an interactive tool for exploring the phasor representation of your FLIM data. You can load a PTU file and an IRF calibration file (Leica IRF, machine-IRF, etc), and the app will compute the phasor histogram and display it alongside the intensity image. You can then place elliptical cursors on the phasor plot to select specific regions of the phasor space, and the corresponding pixels in the intensity image will be highlighted. The statistics for each cursor selection will be printed to the progress log, including the phase lifetime, pixel count, and percentile range. If you place two or more cursors, a two-component decomposition line will be drawn between them, and the component lifetimes and mean fraction will be reported.

You will be able to draw multiple cursors, and the statistics for all of them will be printed to the progress log. The cursors and their statistics are saved as part of the session, so if you save your session and come back to it later, your cursors will still be there.
The session will also be saved, allowing you to pickup where you left off later. This includes the phasor arrays, cursor positions and sizes, and any parameters relevant to the analysis. You can also export the session as a `.npz` file, which can be loaded later to restore the state of the phasor analysis.

---

## Run from source: Requirements & Installation
